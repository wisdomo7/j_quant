...
class NewsCollector:
    ...
    RSS_FEEDS = {
        ...
        "이데일리_시황": {
            "url": "https://rss.edaily.co.kr/edaily_news_finance.xml",
            "publisher": "이데일리",
            "type": "증권"
        },
        # 정치 뉴스 (신규 추가)
        ...
        "이데일리_정치": {
            "url": "https://rss.edaily.co.kr/edaily_news_politics.xml",
            "publisher": "이데일리",
            "type": "정치"
        },
        "한국경제_정치": {
        ...
        "이데일리_국제": {
            "url": "https://rss.edaily.co.kr/edaily_news_international.xml",
            "publisher": "이데일리",
            "type": "국제"
        },
    }
...
if __name__ == "__main__":
    print("=" * 75)
    print("V6.0 NewsCollector 강화판 — 8개 RSS 소스 통합")
    print("=" * 75)
    ...
    print("📊 수집 메타데이터")
    print("─" * 75)
    print(f"  ⏰ 수집 시각: {meta['collection_time']}")
    print(f"  ✅ 활용 출처 ({len(meta['sources_used'])}개):")
    for s in meta['sources_used']:
        print(f"     • {s}")
    if meta['sources_failed']:
        print(f"  ❌ 실패 출처: {meta['sources_failed']}")
    print(f"  📰 수집 뉴스: {meta['total_news']}건 (중복 {meta['duplicates_removed']}건 제거)")
    
    news_list = result['news']
    if news_list:
        print("\n" + "=" * 75)
        print(f"📰 핵심 뉴스 TOP 15 (중요도순)")
        print("=" * 75)
        
        for i, news in enumerate(news_list[:15], 1):
            stars = "★" * news['importance']
            print(f"\n[{i}] {stars} [{news['source_type']}]")
            print(f"  📌 {news['title']}")
