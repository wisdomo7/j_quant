...
V6.0: 블랙리스트 폐지 (전략적 진입 자유)
"""

# ============================================================
# 블랙리스트: V6.0 폐지 - 종목 영구차단 안 함 (정재영님 결정)
# ============================================================
BLACKLIST = {}  # 종목명 기반 차단 폐지 - 펀더멘털 회복 시 어떤 종목이든 진입 가능
...
def analyze_dual_mode(stocks):
    ...
    return {
        "conservative": {
            "long": cons_long[:15], "swing": cons_swing[:15], "day": cons_day[:10],
            "total": len(cons_long) + len(cons_swing) + len(cons_day),
        },
        "aggressive": {
            "long": agg_long[:15], "swing": agg_swing[:15], "day": agg_day[:10],
